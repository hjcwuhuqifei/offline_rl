...
def get_reward(observation, terminal, collision, action):
    reward = action/15 - collision * 100 + (collision - 1) * terminal * 50
...
class OfflineRLtest(gym.Env):
    def __init__(self):
        ...
        self.scenario = pickle.load(open('/home/haojiachen/桌面/offline_rl/offline_/18d37f97-a67a-4e7d-96f2-d1e18832f640' + '.pkl', 'rb'))
        ...
        self.x_threshold = 10000  # 小车x方向最大运动范围
        self.v_threshold = 20
        self.max_a = 20
    ...
    def step(self, action):
        ...
        # 通过环境车辆获取当前观测值
        object_front = [10000, 10000, 10000]
        object_behind = [-10000, 10000, 10000]
        object_left_front = [10000, 10000, 10000]
        object_right_front = [10000, -10000, 10000]
        object_left_behind = [-10000, 10000, 10000]
        object_right_behind = [-10000, -10000, 10000]
        ...
        if observation[4] >= -4:
            ...
            collision = 1
        if observation[7] <= 4 and observation[8] <= 2:
            done = 1
            collision = 1
        if observation[10] <= 4 and observation[11] >= -2:
            done = 1
            collision = 1
        if observation[13] >= -4 and observation[14] <= 2:
            done = 1
            collision = 1
        if observation[16] >= -4 and observation[17] >= -2:
            done = 1
            collision = 1
        ...
        return observation, np.array(get_reward(observation, done, collision, action[0])), np.array(done), {}

    def reset(self):
        self.scenario = pickle.load(open('/home/haojiachen/桌面/offline_rl/offline_/1197d466-f08e-46f0-a37b-42c3425c8380' + '.pkl', 'rb'))
        self.ego_track = self.scenario['EGO']
        self.object_tracks = []
        for key in self.scenario.keys():
            if key != 'EGO' and key != 'states':
